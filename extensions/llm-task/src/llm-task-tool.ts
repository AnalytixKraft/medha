...
import Ajv from "ajv";
// NOTE: This extension is intended to be bundled with Medha.
// When running from source (tests/dev), Medha internals live under src/.
...
export function createLlmTaskTool(api: MedhaPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via medha.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "medha-llm-task-"));
