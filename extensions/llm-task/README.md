...
`medha.invoke --each`) without adding new Medha code per workflow.
...
This extension depends on Medha internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Medha extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.medha/extensions` as a standalone plugin directory.
