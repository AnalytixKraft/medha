...
Medha extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
medha channels login --channel zalouser
...
```bash
medha message send --channel zalouser --target <threadId> --message "Hello from Medha!"
```
...
```bash
medha channels login --channel zalouser              # Login via QR
medha channels login --channel zalouser --account work
medha channels status --probe
medha channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
medha directory self --channel zalouser
medha directory peers list --channel zalouser --query "name"
medha directory groups list --channel zalouser --query "work"
medha directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
medha message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
medha message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `medha channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
medha channels login --channel zalouser --account work
medha message send --channel zalouser --account work --target <id> --message "Hello"
