...
metadata: { "medha": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## Medha Runtime Mapping

- **Task tool** in the upstream spec == Medha `sessions_spawn`
- **File I/O** == Medha `read`/`write`
- **Remote fetch** == Medha `web_fetch` (or `exec` with curl when POST is required)
