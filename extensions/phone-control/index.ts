...
import type { MedhaPluginApi, MedhaPluginService } from "medha/plugin-sdk";
...
function normalizeDenyList(cfg: MedhaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MedhaPluginApi["config"]): string[] {
...
export default function register(api: MedhaPluginApi) {
  ...
  const timerService: MedhaPluginService = {
