...
function resolveMedhaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MEDHA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set MEDHA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMedhaRoot(), "dist", "extensionAPI.js");
