...
import { approveDevicePairing, listDevicePairing } from "medha/plugin-sdk";
...
function resolveGatewayPort(cfg: MedhaPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.MEDHA_GATEWAY_PORT?.trim()) ??
...
async function resolveTailnetHost(api: MedhaPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: MedhaPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: MedhaPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Medha iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Medha iOS app:",
            "",
            "```",
            qrAscii,
            "```",
