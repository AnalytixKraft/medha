...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as MedhaConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as MedhaConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as MedhaConfig);
