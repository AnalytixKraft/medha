...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["medhaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["medha", "pollId"]) ??
    readNestedString(value, ["medha", "poll", "id"]) ??
    readNestedString(value, ["data", "medhaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "medha", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "medha poll vote",
          displayText: "Vote recorded",
          value: { medhaPollId: pollId, pollId },
        },
      },
    ],
  };
