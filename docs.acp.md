...
This document describes how the Medha ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`medha acp` exposes an ACP agent over stdio and forwards prompts to a running
Medha Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Medha Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `medha acp` over stdio.

Example config:

```bash
medha config set gateway.remote.url wss://gateway-host:18789
medha config set gateway.remote.token <token>
```

Example run:

```bash
medha acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
medha acp --session agent:main:main
medha acp --session agent:design:main
medha acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Medha ACP” to start a thread.

## Execution Model

- ACP client spawns `medha acp` and speaks ACP messages over stdio.
...
```bash
medha acp --session agent:main:main
medha acp --session-label "support inbox"
medha acp --reset-session
```
...
`medha acp` resolves the Gateway URL and auth from CLI flags or config:
