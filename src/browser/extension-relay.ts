...
const RELAY_AUTH_HEADER = "x-medha-relay-token";
...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.MEDHA_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
async function looksLikeMedhaRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Medha-Extension-Relay",
          revision: "0",
          userAgent: "Medha-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeMedhaRelay(info.baseUrl))) {
