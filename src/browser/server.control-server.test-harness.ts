...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/medha" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "medha-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "medha",
        profiles: {
          medha: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMedhaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveMedhaUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopMedhaChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.MEDHA_GATEWAY_PORT;
    process.env.MEDHA_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.MEDHA_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.MEDHA_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.MEDHA_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.MEDHA_GATEWAY_TOKEN;
    } else {
      process.env.MEDHA_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.MEDHA_GATEWAY_PASSWORD;
    } else {
      process.env.MEDHA_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
