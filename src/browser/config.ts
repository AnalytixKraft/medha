import type { BrowserConfig, BrowserProfileConfig, MedhaConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "medha" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MEDHA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "medha" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MEDHA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MEDHA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Medha browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for medha/medha CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MedhaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MEDHA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "medha";
