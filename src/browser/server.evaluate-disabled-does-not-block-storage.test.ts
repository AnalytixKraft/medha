  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          medha: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MEDHA_GATEWAY_PORT;
    process.env.MEDHA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MEDHA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MEDHA_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.MEDHA_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.MEDHA_GATEWAY_TOKEN;
    } else {
      process.env.MEDHA_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.MEDHA_GATEWAY_PASSWORD;
    } else {
      process.env.MEDHA_GATEWAY_PASSWORD = prevGatewayPassword;
    }
