...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMedhaTmpDir: vi.fn(() => "/tmp/medha"),
}));
vi.mock("../infra/tmp-medha-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMedhaTmpDir.mockReturnValue("/tmp/medha");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMedhaTmpDir.mockReturnValue("/tmp/medha-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "medha-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMedhaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "medha-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "medha-preferred", "downloads")}${path.sep}`),
    );
  });
