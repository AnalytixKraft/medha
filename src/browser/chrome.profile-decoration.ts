...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".medha-profile-decorated");
}
 ...
 */
export function decorateMedhaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MEDHA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MEDHA_BROWSER_COLOR).toUpperCase();
