...
function telegramCfg(): MedhaConfig {
  return { channels: { telegram: { botToken: "tok" } } } as MedhaConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as MedhaConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as MedhaConfig;
