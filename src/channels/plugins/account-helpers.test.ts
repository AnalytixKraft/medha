...
function cfg(accounts?: Record<string, unknown> | null): MedhaConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as MedhaConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as MedhaConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as MedhaConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as MedhaConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as MedhaConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as MedhaConfig)).toBe("default");
    });
  });
});
