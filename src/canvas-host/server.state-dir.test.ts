...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MEDHA_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Medha Canvas");
    } finally {
      await handler.close();
