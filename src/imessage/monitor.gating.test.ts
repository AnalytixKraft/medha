...
function baseCfg(): MedhaConfig {
  return {
    ...
    messages: {
      groupChat: { mentionPatterns: ["@medha"] },
    },
...
function buildDispatchContextPayload(params: { cfg: MedhaConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@medha hello",
      bodyText: "@medha hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
