...
describe("media store", () => {
  ...
  beforeAll(async () => {
    ...
    home = await fs.mkdtemp(path.join(os.tmpdir(), "medha-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".medha"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.medha${path.sep}media`);
