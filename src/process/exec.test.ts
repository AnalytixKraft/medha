...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["MEDHA_BASE_ENV"]);
    process.env.MEDHA_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.MEDHA_BASE_ENV ?? "") + "|" + (process.env.MEDHA_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { MEDHA_TEST_ENV: "ok" },
        },
      );
