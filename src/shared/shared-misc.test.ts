...
describe("shared/frontmatter", () => {
  ...
  test("resolveMedhaManifestBlock parses JSON5 metadata and picks medha block", () => {
    const frontmatter = {
      metadata: "{ medha: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveMedhaManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveMedhaManifestBlock returns undefined for invalid input", () => {
    expect(resolveMedhaManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveMedhaManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveMedhaManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
