...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-embedded-agent-"));
...
const makeOpenAiConfig = (modelIds: string[]) =>
  ...
  }) satisfies MedhaConfig;

const ensureModels = (cfg: MedhaConfig) => ensureMedhaModelsJson(cfg, agentDir) as unknown;
...
describe("runEmbeddedPiAgent", () => {
  ...
  it("persists prompt transport errors as transcript entries", async () => {
    ...
    const promptErrorEntry = entries.find(
      (entry) => entry.type === "custom" && entry.customType === "medha:prompt-error",
