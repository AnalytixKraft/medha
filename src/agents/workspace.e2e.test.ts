...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MEDHA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MEDHA_HOME: "/srv/medha-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/medha-home"), ".medha", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".medha", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("medha-workspace-");
