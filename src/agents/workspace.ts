...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MEDHA_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".medha", `workspace-${profile}`);
  }
  return path.join(home, ".medha", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".medha";
