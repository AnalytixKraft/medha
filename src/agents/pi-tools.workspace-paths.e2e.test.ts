...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("medha-ws-", async (workspaceDir) => {
      await withTempDir("medha-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMedhaCodingTools({ workspaceDir });
    ...
    await withTempDir("medha-ws-", async (workspaceDir) => {
      await withTempDir("medha-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello medha");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("medha-ws-", async (workspaceDir) => {
      await withTempDir("medha-override-", async (overrideDir) => {
        const tools = createMedhaCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("medha-sandbox-", async (sandboxDir) => {
      await withTempDir("medha-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          containerName: "medha-sbx-test",
          containerWorkdir: "/workspace",
          fsBridge: createHostSandboxFsBridge(sandboxDir),
          docker: {
            image: "medha-sandbox:bookworm-slim",
            containerPrefix: "medha-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createMedhaCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
