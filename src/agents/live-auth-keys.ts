...
const GOOGLE_LIVE_SINGLE_KEY = "MEDHA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MEDHA_LIVE_ANTHROPIC_KEY",
    listVar: "MEDHA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MEDHA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MEDHA_LIVE_${base}_KEY`;
