...
function resolveExecConfig(params: { cfg?: MedhaConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: MedhaConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createMedhaCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createMedhaReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
