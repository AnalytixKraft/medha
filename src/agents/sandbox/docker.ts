...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "medha.sandbox=1");
  args.push("--label", `medha.sessionKey=${params.scopeKey}`);
  args.push("--label", `medha.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `medha.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "medha.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`medha sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`medha sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("medha sandbox recreate --all");
}
