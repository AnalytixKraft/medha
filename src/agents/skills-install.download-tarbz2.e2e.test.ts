...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "medha-skills-install-"));
  try {
...
function restoreMedhaStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.MEDHA_STATE_DIR;
    return;
  }
  process.env.MEDHA_STATE_DIR = originalValue;
}

const originalStateDir = process.env.MEDHA_STATE_DIR;

afterEach(() => {
  restoreMedhaStateDir(originalStateDir);
});
