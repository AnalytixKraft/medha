...
} from "./medha-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateMedhaTools = (typeof import("./medha-tools.js"))["createMedhaTools"];
type CreateMedhaToolsOpts = Parameters<CreateMedhaTools>[0];

async function getSessionsSpawnTool(opts: CreateMedhaToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createMedhaTools } = await import("./medha-tools.js");
  const tool = createMedhaTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("medha-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
