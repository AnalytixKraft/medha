...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { MEDHA_STATE_DIR: process.env.MEDHA_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.MEDHA_STATE_DIR === undefined) {
      delete process.env.MEDHA_STATE_DIR;
    } else {
      process.env.MEDHA_STATE_DIR = snapshot.MEDHA_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "medha-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "medha-memory-flush-"));
  });
