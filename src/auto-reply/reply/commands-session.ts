...
import { scheduleGatewaySigusr1Restart, triggerMedhaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Medha in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMedhaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Medha via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
