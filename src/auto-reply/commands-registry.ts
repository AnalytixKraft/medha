...
export function isCommandEnabled(cfg: MedhaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MedhaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MedhaConfig),
...
export function getCommandDetection(_cfg?: MedhaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MedhaConfig) {
