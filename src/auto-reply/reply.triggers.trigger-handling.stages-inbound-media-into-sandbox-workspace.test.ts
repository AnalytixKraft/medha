...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("medha-triggers-", async (home) => {
      const inboundDir = join(home, ".medha", "media", "inbound");
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("medha-triggers-bypass-", async (home) => {
      // Sensitive host file outside .medha
