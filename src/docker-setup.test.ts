...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "medha-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MEDHA_GATEWAY_TOKEN: "test-token",
    MEDHA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MEDHA_WORKSPACE_DIR: join(sandbox.rootDir, "medha"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        MEDHA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        MEDHA_EXTRA_MOUNTS: undefined,
        MEDHA_HOME_VOLUME: "medha-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MEDHA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MEDHA_EXTRA_MOUNTS=");
    expect(envFile).toContain("MEDHA_HOME_VOLUME=medha-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("medha-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("medha-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MEDHA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
