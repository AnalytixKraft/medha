...
describe("logger helpers", () => {
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `medha-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "medha-2000-01-01.log");
...
function pathForTest() {
  const file = path.join(os.tmpdir(), `medha-log-${crypto.randomUUID()}.log`);
