...
${formatHelpExamples([
  ['medha agent --to +15555550123 --message "status update"', "Start a new session."],
  ['medha agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'medha agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'medha agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['medha agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'medha agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.medha.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.medha.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['medha agents set-identity --agent main --name "Medha" --emoji "🦞"', "Set name + emoji."],
  ["medha agents set-identity --agent main --avatar avatars/medha.png", "Set avatar path."],
  [
    "medha agents set-identity --workspace ~/.medha/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "medha agents set-identity --identity-file ~/.medha/workspace/IDENTITY.md --agent main",
