...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.MEDHA_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.MEDHA_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<MedhaConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Medha docs",
    hasSubcommands: false,
    register: async (program) => {
