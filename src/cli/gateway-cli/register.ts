      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["medha gateway run", "Run the gateway in the foreground."],
            ["medha gateway status", "Show service status and probe reachability."],
            ["medha gateway discover", "Find local and wide-area gateway beacons."],
            ["medha gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.medha.ai/cli/gateway")}\n`,
      ),
  );
