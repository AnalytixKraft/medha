...
import type { MedhaConfig, ConfigFileSnapshot } from "../config/types.medha.js";
...
vi.mock("../infra/medha-root.js", () => ({
  resolveMedhaPackageRoot: vi.fn(),
}));
...
const { resolveMedhaPackageRoot } = await import("../infra/medha-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-update-tests-"));
  });
  ...
  const baseConfig = {} as MedhaConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/medha-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveMedhaPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("medha-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveMedhaPackageRoot).mockReset();
    ...
    vi.mocked(resolveMedhaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("medha");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/medha-restart-test.sh");
  ...
  it("uses stored beta channel when configured", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      config: { update: { channel: "beta" } } as MedhaConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "git",
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveMedhaPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "npm",
      steps: [],
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["MEDHA_UPDATE_IN_PROGRESS"]);
    ...
    try {
      ...
      expect(process.env.MEDHA_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("medha-update-wizard");
    const envSnapshot = captureEnv(["MEDHA_GIT_DIR"]);
    try {
      setTty(true);
      process.env.MEDHA_GIT_DIR = tempDir;
