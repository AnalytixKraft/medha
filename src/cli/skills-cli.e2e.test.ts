...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["MEDHA_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "medha-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "medha-bundled-skills-test-"));
    process.env.MEDHA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
