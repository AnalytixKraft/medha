    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["medha nodes status", "List known nodes with live status."],
          ["medha nodes pairing pending", "Show pending node pairing requests."],
          ['medha nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["medha nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.medha.ai/cli/nodes")}\n`,
    );
