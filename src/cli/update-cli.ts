...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Medha and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["medha update", "Update a source checkout (git)"],
        ["medha update --channel beta", "Switch to beta channel (git + npm)"],
        ["medha update --channel dev", "Switch to dev channel (git + npm)"],
        ["medha update --tag beta", "One-off update to a dist-tag or version"],
        ["medha update --no-restart", "Update without restarting the service"],
        ["medha update --json", "Output result as JSON"],
        ["medha update --yes", "Non-interactive (accept downgrade prompts)"],
        ["medha update wizard", "Interactive update wizard"],
        ["medha --update", "Shorthand for medha update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run medha update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.medha.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["medha update status", "Show channel + version status."],
          ["medha update status --json", "JSON output."],
          ["medha update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.medha.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
