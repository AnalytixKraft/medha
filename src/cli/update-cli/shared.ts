...
import { resolveMedhaPackageRoot } from "../../infra/medha-root.js";
...
const MEDHA_REPO_URL = "https://github.com/medha/medha.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "medha";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("medha@")) {
    return trimmed.slice("medha@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.MEDHA_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MEDHA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MEDHA_GIT_DIR points at a non-git directory: ${params.dir}. Set MEDHA_GIT_DIR to an empty folder or an medha checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MEDHA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "medha.mjs");
