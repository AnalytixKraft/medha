...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "medha-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      MEDHA_SKIP_CHANNELS: "1",
      MEDHA_SKIP_GMAIL_WATCHER: "1",
      MEDHA_SKIP_CRON: "1",
      MEDHA_SKIP_BROWSER_CONTROL_SERVER: "1",
      MEDHA_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "medha-entry-bootstrap.mjs");
