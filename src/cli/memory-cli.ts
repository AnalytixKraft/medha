    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["medha memory status", "Show index and provider status."],
          ["medha memory index --force", "Force a full reindex."],
          ['medha memory search --query "deployment notes"', "Search indexed memory entries."],
          ["medha memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.medha.ai/cli/memory")}\n`,
    );
