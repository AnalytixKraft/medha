...
const SANDBOX_EXAMPLES = {
  main: [
    ["medha sandbox list", "List all sandbox containers."],
    ["medha sandbox list --browser", "List only browser containers."],
    ["medha sandbox recreate --all", "Recreate all containers."],
    ["medha sandbox recreate --session main", "Recreate a specific session."],
    ["medha sandbox recreate --agent mybot", "Recreate agent containers."],
    ["medha sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["medha sandbox list", "List all sandbox containers."],
    ["medha sandbox list --browser", "List only browser containers."],
    ["medha sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["medha sandbox recreate --all", "Recreate all containers."],
    ["medha sandbox recreate --session main", "Recreate a specific session."],
    ["medha sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["medha sandbox recreate --browser --all", "Recreate only browser containers."],
    ["medha sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["medha sandbox explain", "Show effective sandbox config."],
    ["medha sandbox explain --session agent:main:main", "Explain a specific session."],
    ["medha sandbox explain --agent work", "Explain an agent sandbox."],
    ["medha sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.medha.ai/cli/sandbox")}\n`,
    )
    .action(() => {
