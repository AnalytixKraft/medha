...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("medha node install"), formatCliCommand("medha node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("medha node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    MEDHA_LOG_PREFIX: baseEnv.MEDHA_LOG_PREFIX ?? "node",
