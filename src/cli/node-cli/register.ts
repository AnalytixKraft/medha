    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "medha node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["medha node status", "Check node host service status."],
          ["medha node install", "Install the node host service."],
          ["medha node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.medha.ai/cli/node")}\n`,
    );
