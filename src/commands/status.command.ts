...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Medha status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("medha security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("medha security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.medha.ai/faq");
  runtime.log("Troubleshooting: https://docs.medha.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("medha status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("medha logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("medha status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("medha gateway probe")}`);
  }
}
