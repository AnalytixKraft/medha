...
async function runRepair(cfg: MedhaConfig) {
...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      MEDHA_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service MEDHA_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      MEDHA_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses MEDHA_GATEWAY_TOKEN when config token is missing", async () => {
    const previousToken = process.env.MEDHA_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.MEDHA_GATEWAY_TOKEN = previousToken;
      }
    }
  });
});
