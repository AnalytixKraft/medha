...
function resolveAgentName(cfg: MedhaConfig, agentId: string) {
...
function resolveAgentModel(cfg: MedhaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MedhaConfig): AgentSummary[] {
