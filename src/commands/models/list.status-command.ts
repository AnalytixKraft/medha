...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMedhaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("medha models auth setup-token")}\` or \`${formatCliCommand("medha configure")}\`.`
          : `Run \`${formatCliCommand("medha configure")}\` or set an API key env var.`;
