...
export async function loadValidConfigOrThrow(): Promise<MedhaConfig> {
...
export async function updateConfig(
  mutator: (cfg: MedhaConfig) => MedhaConfig,
): Promise<MedhaConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: MedhaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MedhaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("medha agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
