...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMedhaAgentDir: vi.fn().mockReturnValue("/tmp/medha-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/medha-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/medha-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMedhaAgentDir: mocks.resolveMedhaAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/medha-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMedhaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/medha-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/medha-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/medha-agent-custom");
