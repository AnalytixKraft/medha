...
const SEARCH_TOOL = "https://docs.medha.ai/mcp.SearchMedha";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.medha.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('medha docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.medha.ai/");
      runtime.log(`Search: ${formatCliCommand('medha docs "your query"')}`);
    }
    return;
  }
