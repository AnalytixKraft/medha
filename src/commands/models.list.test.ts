...
const ensureMedhaModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/medha-agent/auth.json" });
const resolveMedhaAgentDir = vi.fn().mockReturnValue("/tmp/medha-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/medha-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureMedhaModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/medha-agent");
  });
