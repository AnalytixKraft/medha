...
function resolveMode(cfg: MedhaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveMedhaPackageRoot({
  ...
  let cfg: MedhaConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("medha configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("medha config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("medha setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("medha doctor --fix")}" to apply changes.`);
  }
