...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MEDHA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MEDHA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MEDHA_GATEWAY_TOKEN");
    expect(message).not.toContain("MEDHA_GATEWAY_PASSWORD");
  });
