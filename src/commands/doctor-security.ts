...
import type { MedhaConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { resolveDmAllowState } from "../security/dm-policy-shared.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: MedhaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("medha security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("medha configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("medha config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("medha doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "medha config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("medha config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('medha config set session.dmScope "per-channel-peer"') +
