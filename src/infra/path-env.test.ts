...
let ensureMedhaCliOnPath: typeof import("./path-env.js").ensureMedhaCliOnPath;

describe("ensureMedhaCliOnPath", () => {
  const envKeys = [
    "PATH",
    "MEDHA_PATH_BOOTSTRAPPED",
    "MEDHA_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureMedhaCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling medha exists", () => {
    const tmp = abs("/tmp/medha-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "medha");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.MEDHA_PATH_BOOTSTRAPPED;

    ensureMedhaCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.MEDHA_PATH_BOOTSTRAPPED = "1";
    ensureMedhaCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/medha-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "medha");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/medha-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "medha");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "medha");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/medha-path/case-linuxbrew");
