...
describe("resolveEffectiveHomeDir", () => {
  it("prefers MEDHA_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands MEDHA_HOME when set to ~", () => {
    const env = {
      MEDHA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MEDHA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MEDHA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MEDHA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MEDHA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { MEDHA_HOME: "/srv/medha-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/medha-home")}/x`);
  });
