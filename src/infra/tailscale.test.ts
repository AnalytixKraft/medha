...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.MEDHA_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.MEDHA_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.MEDHA_TEST_TAILSCALE_BINARY;
    } else {
      process.env.MEDHA_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
