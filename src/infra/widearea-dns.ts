...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MEDHA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*medha-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "medha";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "medha");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "medha-gw");
  ...
  records.push(`_medha-gw._tcp IN PTR ${instanceLabel}._medha-gw._tcp`);
  records.push(`${instanceLabel}._medha-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._medha-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; medha-content-hash: ${contentHash}\n${contentBody}`;
}
