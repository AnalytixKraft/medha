...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__medha_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "medha-root");
...
describe("resolveMedhaPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveMedhaPackageRootSync } = await import("./medha-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "medha");
    const pkgRoot = path.join(project, "node_modules", "medha");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "medha" }));

    expect(resolveMedhaPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "medha");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "medha.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "medha" }));

    expect(resolveMedhaPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveMedhaPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-medha package roots", async () => {
    const { resolveMedhaPackageRootSync } = await import("./medha-root.js");

    const pkgRoot = fx("not-medha");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-medha" }));

    expect(resolveMedhaPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveMedhaPackageRoot } = await import("./medha-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "medha" }));

    await expect(resolveMedhaPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
