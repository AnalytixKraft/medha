...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MedhaConfig | null = null;
  ...
  get(key: string, cfg: MedhaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MedhaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MedhaConfig): void {
