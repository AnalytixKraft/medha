...
import { POSIX_MEDHA_TMP_DIR, resolvePreferredMedhaTmpDir } from "./tmp-medha-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMedhaTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `medha-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMedhaTmpDir({
    accessSync,
...
describe("resolvePreferredMedhaTmpDir", () => {
  it("prefers /tmp/medha when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MEDHA_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/medha when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MEDHA_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/medha when /tmp/medha is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/medha when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/medha is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/medha is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/medha is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
