...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MedhaConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MedhaConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: MedhaConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: MedhaConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: MedhaConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MedhaConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MedhaConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
