...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("medha-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-medha-archive.tgz");
  ...
  it("rejects unsupported archive extensions", async () => {
    const dir = await createTempDir("medha-install-source-utils-");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using the final non-empty stdout line", async () => {
    const cwd = await createTempDir("medha-install-source-utils-");
    runCommandWithTimeoutMock.mockResolvedValue({
      stdout: "npm notice created package\nmedha-plugin-1.2.3.tgz\n",
      stderr: "",
      code: 0,
      signal: null,
      killed: false,
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "medha-plugin-1.2.3.tgz"),
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "medha-plugin@1.2.3", "--ignore-scripts"],
