...
function candidateBinDirs(opts: EnsureMedhaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `medha` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "medha");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MEDHA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "medha"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `medha` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMedhaCliOnPath(opts: EnsureMedhaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MEDHA_PATH_BOOTSTRAPPED)) {
    return;
  }
