...
describe("update-startup", () => {
  ...
  let resolveMedhaPackageRoot: (typeof import("./medha-root.js"))["resolveMedhaPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "MEDHA_STATE_DIR");
    prevStateDir = process.env.MEDHA_STATE_DIR;
    process.env.MEDHA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveMedhaPackageRoot } = await import("./medha-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveMedhaPackageRoot).mockResolvedValue("/opt/medha");
