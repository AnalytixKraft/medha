...
export function triggerMedhaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MEDHA_SYSTEMD_UNIT,
        process.env.MEDHA_PROFILE,
      );
  ...
  const label =
    process.env.MEDHA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MEDHA_PROFILE);
