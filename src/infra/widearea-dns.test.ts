...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "medha.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (Medha)",
    ...
    expect(txt).toContain(`$ORIGIN medha.internal.`);
    ...
    expect(txt).toContain(`_medha-gw._tcp IN PTR studio-london._medha-gw._tcp`);
    expect(txt).toContain(`studio-london._medha-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Medha)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/medha`);
  });
