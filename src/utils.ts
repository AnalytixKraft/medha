...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".medha");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.MEDHA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$MEDHA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via MEDHA_STATE_DIR.
