...
declare const __MEDHA_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __MEDHA_VERSION__ === "string" && __MEDHA_VERSION__) ||
  process.env.MEDHA_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `medha --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.MEDHA_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.MEDHA_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (MEDHA_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
