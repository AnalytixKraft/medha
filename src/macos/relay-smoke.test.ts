...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { MEDHA_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MEDHA_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports MEDHA_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { MEDHA_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MEDHA_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { MEDHA_SMOKE: "nope" })).toBe("qr");
  });
});
