...
import { createMedhaTools } from "../agents/medha-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-medha-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-medha-account-id")?.trim() || undefined;
  ...
  const allTools = createMedhaTools({
