...
export function resolveHooksConfig(cfg: MedhaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MedhaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-medha-token"] === "string"
      ? req.headers["x-medha-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
