...
const connectNodeClient = async (params: {
  ...
  const token = process.env.MEDHA_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("MEDHA_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".medha", "restart-sentinel.json");
