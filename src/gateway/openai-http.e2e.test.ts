    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "medha", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-medha-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-medha-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "medha", messages: [{ role: "user", content: "hi" }] },
          {
            "x-medha-agent-id": "beta",
            "x-medha-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
