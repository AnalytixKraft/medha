...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.MEDHA_STATE_DIR;
  previousConfigPath = process.env.MEDHA_CONFIG_PATH;
  previousSkipBrowserControl = process.env.MEDHA_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.MEDHA_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.MEDHA_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.MEDHA_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.MEDHA_SKIP_CHANNELS;
  previousSkipProviders = process.env.MEDHA_SKIP_PROVIDERS;
  previousSkipCron = process.env.MEDHA_SKIP_CRON;
  previousMinimalGateway = process.env.MEDHA_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "medha-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MEDHA_STATE_DIR = path.join(tempHome, ".medha");
  delete process.env.MEDHA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.MEDHA_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.MEDHA_SKIP_GMAIL_WATCHER = "1";
  process.env.MEDHA_SKIP_CANVAS_HOST = "1";
  process.env.MEDHA_SKIP_CHANNELS = "1";
  process.env.MEDHA_SKIP_PROVIDERS = "1";
  process.env.MEDHA_SKIP_CRON = "1";
  process.env.MEDHA_TEST_MINIMAL_GATEWAY = "1";
  process.env.MEDHA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "medha-test-no-bundled-extensions")
    : "medha-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "medha-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".medha-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.MEDHA_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.MEDHA_CONFIG_PATH;
    } else {
      process.env.MEDHA_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.MEDHA_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.MEDHA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.MEDHA_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MEDHA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.MEDHA_SKIP_CANVAS_HOST;
    } else {
      process.env.MEDHA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.MEDHA_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.MEDHA_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.MEDHA_SKIP_CHANNELS;
    } else {
      process.env.MEDHA_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.MEDHA_SKIP_PROVIDERS;
    } else {
      process.env.MEDHA_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.MEDHA_SKIP_CRON;
    } else {
      process.env.MEDHA_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.MEDHA_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.MEDHA_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MEDHA_GATEWAY_TOKEN"]);
  const prev = process.env.MEDHA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    delete process.env.MEDHA_GATEWAY_TOKEN;
  } else {
    process.env.MEDHA_GATEWAY_TOKEN = fallbackToken;
  }
