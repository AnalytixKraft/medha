...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MedhaPluginDefinition;
  register?: MedhaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MedhaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MedhaPluginDefinition;
...
export function loadMedhaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverMedhaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "medha/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "medha/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MedhaPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as MedhaPluginModule;
    } catch (err) {
