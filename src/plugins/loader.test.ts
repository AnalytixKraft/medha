...
import { loadMedhaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `medha-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.MEDHA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "medha.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.MEDHA_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadMedhaPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.MEDHA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMedhaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadMedhaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadMedhaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
