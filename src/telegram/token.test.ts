...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "medha-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.MEDHA_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "medha-telegram-"));
  process.env.MEDHA_STATE_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (previous === undefined) {
      ...
      process.env.MEDHA_STATE_DIR = previous;
    }
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as MedhaConfig;
    const res = resolveTelegramToken(cfg);
