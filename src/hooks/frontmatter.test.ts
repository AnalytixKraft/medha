...
`;
    ...
    expect(parsed.medha.emoji).toBe("💾");
    expect(parsed.medha.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.medha.emoji).toBe("📝");
    expect(parsed.medha.events).toEqual(["command"]);
    expect(parsed.medha.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.medha.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"medha": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"medha": {"events": ["test"]}}');
  });
...
describe("resolveMedhaMetadata", () => {
  it("extracts medha metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMedhaMetadata(frontmatter);
  ...
  it("returns undefined when medha key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        medha: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Medha" },
            { id: "npm", kind: "npm", package: "@medha/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@medha/hook");
  });
...
homepage: https://docs.medha.ai/automation/hooks#session-memory
metadata:
  {
    "medha":
      {
...
`;
    ...
    const medha = resolveMedhaMetadata(frontmatter);
    expect(medha).toBeDefined();
    expect(medha?.emoji).toBe("💾");
    expect(medha?.events).toEqual(["command:new"]);
    expect(medha?.requires?.config).toEqual(["workspace.dir"]);
    expect(medha?.install?.[0].kind).toBe("bundled");
  });
