...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.MEDHA_BUNDLED_HOOKS_DIR;
    process.env.MEDHA_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.MEDHA_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MEDHA_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
