...
This directory contains hooks that ship with Medha. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.medha/workspace`)

**Enable**:

```bash
medha hooks enable session-memory
```
...
```bash
medha hooks enable bootstrap-extra-files
```
...
**Output**: `~/.medha/logs/commands.log`

**Enable**:

```bash
medha hooks enable command-logger
```
...
homepage: https://docs.medha.ai/automation/hooks#my-hook
metadata:
  { "medha": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.medha/hooks/` (shared across workspaces)
...
```bash
medha hooks info session-memory
```
...
```bash
medha hooks enable session-memory
medha hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.medha/medha.json`:
...
2. Restart gateway: `pkill -9 -f 'medha.*gateway' && pnpm medha gateway`
3. Enable the hook: `medha hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.medha.ai/automation/hooks
