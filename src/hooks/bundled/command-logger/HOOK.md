...
homepage: https://docs.medha.ai/automation/hooks#command-logger
metadata:
  {
    "medha":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.medha/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.medha/logs/commands.log ~/.medha/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/medha`:
   ```
   /home/username/.medha/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.medha/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.medha/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.medha/logs/commands.log | jq .
```
