...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => MedhaConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("medha-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): MedhaConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
