...
import type { MedhaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareMedhaVersions } from "./version.js";
...
function coerceConfig(value: unknown): MedhaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MedhaConfig;
}
...
function stampConfigVersion(cfg: MedhaConfig): MedhaConfig {
...
function warnIfConfigFromFuture(cfg: MedhaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareMedhaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Medha (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MedhaConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MedhaConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MedhaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MedhaConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MedhaConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MEDHA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MEDHA_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MEDHA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MEDHA_WATCH_SESSION === "string" &&
        deps.env.MEDHA_WATCH_SESSION.trim().length > 0
          ? deps.env.MEDHA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MEDHA_WATCH_COMMAND === "string" &&
        deps.env.MEDHA_WATCH_COMMAND.trim().length > 0
          ? deps.env.MEDHA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `MEDHA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MEDHA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MEDHA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): MedhaConfig {
