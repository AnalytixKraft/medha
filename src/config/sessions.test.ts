...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-medha",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-medha#general");
  });
  ...
  it("derives session transcripts dir from MEDHA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MEDHA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.MEDHA_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.MEDHA_STATE_DIR;
    const stateDir = path.resolve("/home/user/.medha");
  ...
  it("resolves cross-agent paths when MEDHA_STATE_DIR differs from stored paths", () => {
    const prev = process.env.MEDHA_STATE_DIR;
    process.env.MEDHA_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.MEDHA_STATE_DIR;
    process.env.MEDHA_STATE_DIR = path.resolve("/home/user/.medha");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
