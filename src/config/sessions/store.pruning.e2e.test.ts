...
// Keep integration tests deterministic: never read a real medha.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "medha-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.MEDHA_SESSION_CACHE_TTL_MS;
    process.env.MEDHA_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.MEDHA_SESSION_CACHE_TTL_MS;
    } else {
      process.env.MEDHA_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
