...
import type { MedhaConfig, ConfigValidationIssue } from "./types.js";
import { MedhaSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: MedhaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MedhaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MedhaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MedhaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MedhaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as MedhaConfig,
  };
}
