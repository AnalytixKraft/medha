...
export const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Medha writes the config.",
  ...
  "gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /medha).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $MEDHA_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "tools.web.search.openai.apiKey":
    "OpenAI API key (fallback: OPENAI_API_KEY env var, then auth profiles).",
  "tools.web.search.openai.model": 'OpenAI model override for web_search (default: "gpt-5").',
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.medha/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for Medha embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `medha plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.medha/extensions/<id>).",
