...
import type { ConfigFileSnapshot } from "./types.medha.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.medha/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/medha/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/medha/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/medha/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/medha/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MedhaSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MedhaConfig";
    const hints = mapSensitivePaths(MedhaSchema, "", {});
