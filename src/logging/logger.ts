...
import type { MedhaConfig } from "../config/types.js";
import { resolvePreferredMedhaTmpDir } from "../infra/tmp-medha-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredMedhaTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "medha.log"); // legacy single-file path

const LOG_PREFIX = "medha";
...
function resolveSettings(): ResolvedSettings {
  let cfg: MedhaConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => MedhaConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MEDHA_TEST_FILE_LOG !== "1" ? "silent" : "info";
