...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "medha-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MEDHA_HOME for tilde expansion", () => {
    vi.stubEnv("MEDHA_HOME", "/srv/medha-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/medha-home", "cron", "jobs.json"));
  });
});
