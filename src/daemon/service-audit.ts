...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.MEDHA_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service MEDHA_GATEWAY_TOKEN does not match gateway.auth.token in medha.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `medha gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
