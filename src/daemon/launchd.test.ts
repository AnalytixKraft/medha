...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.medha.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", MEDHA_PROFILE: "default" },
    });
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when MEDHA_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.medha.gateway.plist",
    );
  });

  it("uses profile-specific label when MEDHA_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", MEDHA_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.medha.jbphoenix.plist",
    );
  });

  it("prefers MEDHA_LAUNCHD_LABEL over MEDHA_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      MEDHA_PROFILE: "jbphoenix",
      MEDHA_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from MEDHA_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      MEDHA_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty MEDHA_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      MEDHA_PROFILE: "myprofile",
      MEDHA_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.medha.myprofile.plist",
    );
  });
});
