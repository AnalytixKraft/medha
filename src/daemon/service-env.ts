...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.MEDHA_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.MEDHA_STATE_DIR;
  const configPath = env.MEDHA_CONFIG_PATH;
  ...
  return {
    ...
    MEDHA_PROFILE: profile,
    MEDHA_STATE_DIR: stateDir,
    MEDHA_CONFIG_PATH: configPath,
    MEDHA_GATEWAY_PORT: String(port),
    MEDHA_GATEWAY_TOKEN: token,
    MEDHA_LAUNCHD_LABEL: resolvedLaunchdLabel,
    MEDHA_SYSTEMD_UNIT: systemdUnit,
    MEDHA_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    MEDHA_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    MEDHA_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    MEDHA_STATE_DIR: stateDir,
    MEDHA_CONFIG_PATH: configPath,
    MEDHA_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    MEDHA_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    MEDHA_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    MEDHA_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    MEDHA_LOG_PREFIX: "node",
    MEDHA_SERVICE_MARKER: NODE_SERVICE_MARKER,
    MEDHA_SERVICE_KIND: NODE_SERVICE_KIND,
    MEDHA_SERVICE_VERSION: VERSION,
  };
}
