...
function summarizeGroupPolicy(cfg: MedhaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: MedhaConfig): boolean {
...
function collectModels(cfg: MedhaConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: MedhaConfig): Set<string> {
  const baseCfg: MedhaConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: MedhaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: MedhaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: MedhaConfig): boolean {
...
function isBrowserEnabled(cfg: MedhaConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: MedhaConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep MEDHA_STATE_DIR on a local-only volume and re-run "${formatCliCommand("medha security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer MEDHA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const medhaGatewayToken =
    typeof env.MEDHA_GATEWAY_TOKEN === "string" && env.MEDHA_GATEWAY_TOKEN.trim()
      ? env.MEDHA_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-medha-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: MedhaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
