 ...
 * `medha update`. Use it to verify the completion installation flow
...
\`medha update\`, \`medha doctor\`, and \`medha onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Medha Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`medha completion --install\` later to enable.`));
    return;
  }
