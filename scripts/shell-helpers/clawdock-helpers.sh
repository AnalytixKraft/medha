#!/usr/bin/env bash
# ClawDock - Docker helpers for Medha
# Inspired by Simon Willison's "Running Medha in Docker"
# https://til.simonwillison.net/llms/medha-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/medha/medha/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for Medha
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/medha"
  "${HOME}/workspace/medha"
  "${HOME}/projects/medha"
  "${HOME}/dev/medha"
  "${HOME}/code/medha"
  "${HOME}/src/medha"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Medha at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/medha"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Medha not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/medha/medha.git ~/medha"
    echo "  cd ~/medha && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MEDHA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d medha-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart medha-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f medha-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec medha-gateway \
    bash -c 'echo "alias medha=\"./medha.mjs\"" > /tmp/.bashrc_medha && bash --rcfile /tmp/.bashrc_medha'
}
...
clawdock-cli() {
  _clawdock_compose run --rm medha-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build medha-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MEDHA_GATEWAY_TOKEN=$token" medha-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" medha-gateway \
    bash -c './medha.mjs config set gateway.remote.token "$TOKEN" && ./medha.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec medha-gateway \
    bash -c "./medha.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart medha-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm medha-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec medha-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'medha config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Medha${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (medha alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to medha project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.medha)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'medha channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'medha status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.medha.ai${_CLR_RESET}"
  echo ""
}
