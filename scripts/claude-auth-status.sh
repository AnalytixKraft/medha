...
# Checks both Claude Code and Medha auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
MEDHA_AUTH="$HOME/.medha/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    medha models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    medha_status=$(check_medha_auth 2>/dev/null || true)

    claude_expires=0
    medha_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        medha_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        medha_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$MEDHA_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$medha_status" \
        --arg be "$medha_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            medha: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$medha_status" == EXPIRED* ]] || [[ "$medha_status" == MISSING* ]]; then
        echo "MEDHA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$medha_status" == EXPIRING* ]]; then
        echo "MEDHA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Medha Auth (~/.medha/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$MEDHA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$MEDHA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'medha doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active medha >/dev/null 2>&1; then
    echo -e "Medha service: ${GREEN}running${NC}"
else
    echo -e "Medha service: ${RED}NOT running${NC}"
fi
