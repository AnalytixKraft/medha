...
ZIP=${1:?"Usage: $0 Medha-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/medha/medha/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Medha-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/medha/medha/releases/download/v${VERSION}/"}
