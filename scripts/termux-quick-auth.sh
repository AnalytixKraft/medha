...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/medha/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Medha Re-Auth" -c "After getting key, run: ssh $SERVER '~/medha/scripts/mobile-reauth.sh'" --id medha-auth
        ;;
    *)
