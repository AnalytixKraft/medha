...
# Build and bundle Medha into a minimal .app we can open.
# Outputs to dist/Medha.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Medha.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Medha"
BUNDLE_ID="${BUNDLE_ID:-ai.medha.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/medha/medha/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Medha/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MedhaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MedhaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Medha"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Medha"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Medha"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Medha" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Medha/Resources/Medha.icns" "$APP_ROOT/Contents/Resources/Medha.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Medha/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MedhaKit resources"
MEDHAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MedhaKit_MedhaKit.bundle"
if [ -d "$MEDHAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MedhaKit_MedhaKit.bundle"
  cp -R "$MEDHAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MedhaKit_MedhaKit.bundle"
else
  echo "WARN: MedhaKit resource bundle not found at $MEDHAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Medha"
killall -q Medha 2>/dev/null || true
