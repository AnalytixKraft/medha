...
INSTALL_URL="${MEDHA_INSTALL_URL:-https://medha.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MEDHA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MEDHA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="medha"
PACKAGE_NAME="${MEDHA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MEDHA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MEDHA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MEDHA_INSTALL_LATEST_OUT:-}"
fi
