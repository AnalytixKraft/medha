...
docker run --rm -t \
  ...
  -e MEDHA_LIVE_TEST=1 \
  -e MEDHA_LIVE_GATEWAY_MODELS="${MEDHA_LIVE_GATEWAY_MODELS:-${CLAWDBOT_LIVE_GATEWAY_MODELS:-all}}" \
  -e MEDHA_LIVE_GATEWAY_PROVIDERS="${MEDHA_LIVE_GATEWAY_PROVIDERS:-${CLAWDBOT_LIVE_GATEWAY_PROVIDERS:-}}" \
  -e MEDHA_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${MEDHA_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}}" \
