#!/bin/bash
# Setup Medha Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'medha-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Medha message: Send warning via Medha itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/medha-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/medha-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/medha-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now medha-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status medha-auth-monitor.timer"
echo ""
echo "Quick commands:"
