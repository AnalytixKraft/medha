...
IMAGE_NAME="medha-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="medha-net-e2e-$$"
GW_NAME="medha-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MEDHA_GATEWAY_TOKEN=$TOKEN" \
	  -e "MEDHA_SKIP_CHANNELS=1" \
	  -e "MEDHA_SKIP_GMAIL_WATCHER=1" \
	  -e "MEDHA_SKIP_CRON=1" \
	  -e "MEDHA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
