...
#
# Suggested cron: */30 * * * * /home/admin/medha/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Medha notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., medha-alerts)
...
STATE_FILE="$HOME/.medha/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Medha if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use medha
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Medha to $NOTIFY_PHONE..."
            medha send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Medha is down. Run: ssh l36 '~/medha/scripts/mobile-reauth.sh'" "urgent"
    exit 1
