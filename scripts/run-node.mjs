...
const shouldBuild = (deps) => {
  if (deps.env.MEDHA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MEDHA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[medha] ${message}\n`);
};

const runMedha = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["medha.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
