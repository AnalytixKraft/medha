---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Medha deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Medha cron scheduling for periodic checks, or version status checks on a machine running Medha (laptop, workstation, Pi, VPS).
---

# Medha Host Hardening

## Overview

Assess and harden the host running Medha, then align it to a user-defined risk tolerance without breaking access. Use Medha security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Medha changes the host firewall, SSH, or OS updates; it does not.
...
5. Medha gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Medha can access sensitive data.
...
### 2) Run Medha security audits (read-only)

As part of the default read-only checks, run `medha security audit --deep`. Only offer alternatives if the user requests them:

1. `medha security audit` (faster, non-probing)
2. `medha security audit --json` (structured output)

Offer to apply Medha safe defaults (numbered):

1. `medha security audit --fix`

Be explicit that `--fix` only tightens Medha defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Medha version/update status (read-only)

As part of the default read-only checks, run `medha update status`.
...
- Credential hygiene notes (location of Medha creds, prefer disk encryption)
...
After Medha install or first hardening pass, run at least one baseline audit and version check:

- `medha security audit`
- `medha security audit --deep`
- `medha update status`

Ongoing monitoring is recommended. Use the Medha cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `medha cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `medha update status`
...
Before creating, `medha cron list` and match on exact `name`. If found, `medha cron edit <id> ...`.
If not found, `medha cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `medha update status` (preferred for source checkouts and channels)
2. `npm view medha version` (published npm version)

## Medha command accuracy

Use only supported commands and flags:

- `medha security audit [--deep] [--fix] [--json]`
- `medha status` / `medha status --deep`
- `medha health --json`
- `medha update status`
- `medha cron add|list|runs|run`

Do not invent CLI flags or imply Medha enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Medha compaction:
